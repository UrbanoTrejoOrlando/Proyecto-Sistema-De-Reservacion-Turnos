...
export const AdminStats: React.FC = () => {
  const { appointments, services, getAppointmentStats } = useAppointments();
  ...
  const todayAppointments = appointments.filter(app => app.date === today && app.status === 'confirmed').length;
  ...
  const thisWeekAppointments = appointments.filter(app => {
    ...
    return appDate >= weekStart && appDate <= weekEnd && app.status === 'confirmed';
  }).length;
  ...
  const appointmentsByService = services.map(service => {
    const serviceAppointments = appointments.filter(app => app.serviceId === service.id);
    
    return {
      service: service.name,
      total: serviceAppointments.length,
      confirmed: serviceAppointments.filter(app => app.status === 'confirmed').length,
      cancelled: serviceAppointments.filter(app => app.status === 'cancelled').length
    };
                ...
                <div className="flex-1 mx-4">
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div 
                      ...
                      style={{ width: `${Math.max((count / Math.max(...Object.values(stats.appointmentsByDay))) * 100, 5)}%` }}
                    ></div>
                  </div>
                </div>
